...
L'esquema **3.0** conté els blocs `world`, `population`, `policy`, `observations`, `energy`,
...
- `evolution`: edats i taxes, sigmes i límits explícits i independents per a pesos i biaixos.
  `weight_abs_limit` ha de cobrir tot el suport Glorot de la topologia v1. Els dos límits tenen
  una cota superior conservadora perquè la suma dels 375 deltes continuï representable en
  `float32`.
