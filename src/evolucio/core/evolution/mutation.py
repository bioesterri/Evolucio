...
def _mutate_leaf(
    ...
    perturbed = jnp.where(selected, inherited + noise, inherited)
    candidate = jnp.clip(perturbed, -abs_limit, abs_limit)
    apply_mask = valid_newborn.reshape((-1,) + (1,) * len(element_shape))
    mutated = jnp.where(apply_mask, candidate, inherited)
    zero_delta = jnp.zeros_like(inherited)  # pyright: ignore[reportUnknownMemberType]
    delta = jnp.where(apply_mask, jnp.abs(mutated - inherited), zero_delta)
    return mutated, selected, delta
...
def mutate_newborn_genomes(
    *,
    ...
    valid_genome_id = population.genome_id >= 0
    consistent = population.alive & valid_genome_id & finite & weights_bounded & biases_bounded
    valid_newborn = newborn_mask & consistent
    invalid_newborn = newborn_mask & ~consistent
    zero_genome_ids = jnp.zeros_like(  # pyright: ignore[reportUnknownMemberType]
        population.genome_id
    )
    safe_genome_ids = jnp.where(valid_genome_id, population.genome_id, zero_genome_ids)
    entity_keys = derive_entity_keys(genome_mutation_key, safe_genome_ids)
