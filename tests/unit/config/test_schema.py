...
def test_schema_snapshot() -> None:
    ...
    assert CONFIG_SCHEMA_VERSION == "3.0"
    ...
    assert json.loads(Path("docs/schemas/experiment-config-v3.0.json").read_text()) == schema


def test_previous_schema_snapshot_remains_published() -> None:
    """Publishing 3.0 must not reinterpret the auditable 2.2 contract."""
    previous = json.loads(Path("docs/schemas/experiment-config-v2.2.json").read_text())

    assert previous["properties"]["schema_version"]["const"] == "2.2"
    assert (
