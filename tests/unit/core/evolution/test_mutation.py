...
def test_nonfinite_invalid_newborn_has_a_finite_zero_summary() -> None:
    genomes = create_empty_genome_batch(4)
    genomes = type(genomes)(
        layer1=type(genomes.layer1)(
            weight=genomes.layer1.weight.at[1, 0, 0].set(jnp.nan), bias=genomes.layer1.bias
        ),
        layer2=genomes.layer2,
    )
    result = mutate(genomes, population(), jnp.array([False, True, False, False]))

    assert bool(jnp.isnan(result.genomes.layer1.weight[1, 0, 0]))
    assert int(result.invalid_newborn_genome_count) == 1
    assert int(result.selected_parameter_count[1]) == 0
    assert int(result.effective_parameter_count[1]) == 0
    assert float(result.sum_abs_delta[1]) == 0.0
    assert float(result.max_abs_delta[1]) == 0.0
    assert bool(jnp.all(jnp.isfinite(result.sum_abs_delta)))
    assert bool(jnp.all(jnp.isfinite(result.max_abs_delta)))


def test_invalid_genome_identity_is_diagnosed_without_entering_rng_derivation() -> None:
    genomes = create_empty_genome_batch(4)
    state = population()
    state = type(state)(
        **{
            field: (-jnp.ones_like(value) if field == "genome_id" else value)
            for field, value in vars(state).items()
        }
    )
    result = mutate(genomes, state, jnp.array([False, True, False, False]))
...
def test_intermediate_rate_selects_a_broadly_compatible_parameter_fraction() -> None:
    capacity = 64
    result = mutate(
        create_empty_genome_batch(capacity),
        population(capacity),
        jnp.ones(capacity, dtype=jnp.bool_),
        weight_rate=0.1,
        bias_rate=0.1,
    )
    selected = int(jnp.sum(result.selected_parameter_count))
    parameter_count = capacity * 375

    assert parameter_count * 0.05 < selected < parameter_count * 0.15
